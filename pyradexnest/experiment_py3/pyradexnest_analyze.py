...
1) meas=pickle.load(open("measdata.pkl","rb"),encoding='latin1') #line 53
2) meas['mol']=np.array([mea.decode('UTF-8') for mea in meas['mol']]) #line 54
'''
#
import pymultinest
...
meas=pickle.load(open("measdata.pkl","rb"),encoding='latin1')
meas['mol']=np.array([mea.decode('UTF-8') for mea in meas['mol']])
...
if a.post_file=='chains/1-post_seperate.dat': a.post_file='chains/1-post_separate.dat'
        ...
        for p in range(n_params):
            ...
            if mode != 'all': 
                ...
                try:
                    ...
                    dists[mode][p][1,:]*=np.exp(np.array(s['modes'][mode]['local log-evidence']))/np.exp(s['global evidence'])
