...
Hao He
June 17th, 2019
Change the script from python 2 to python 3. 
1)  ok=(meas['flux']!=0) & (meas['mol']==mol) #[meas['flux']!=0] # line 900
    ul=(meas['flux']==0) & (meas['mol']==mol) #[meas['flux']==0]
2)  ok2=(meas2['flux']!=0) & (meas2['mol']==mol) #[meas2['flux']!=0] # line 996
    ul2=(meas2['flux']==0) & (meas2['mol']==mol) #[meas2['flux']==0]
'''
...
def define_plotting_multimol(n_comp,n_mol,n_dims,n_sec,n_params,sled_to_j,lw,compare=False):
    ...
    if n_comp==1:
        ...
        if sled_to_j: 
            for m in range(n_mol): list(map(parameters.append,["k"+str(i)+'mol'+str(m) for i in range(1,sled_to_j+1)]))
        ...
        plotinds.append(list(range(n_dims+np.sum(n_sec),n_dims+np.sum(n_sec)+sled_to_j,1))) # Flux likelihoods, Main molecule
        ...
        if sled_to_j:
            for m in range(n_mol): list(map(parameters.append,["k"+str(i)+'mol'+str(m)+"c" for i in range(1,sled_to_j+1)])) # Cold
            for m in range(n_mol): list(map(parameters.append,["k"+str(i)+'mol'+str(m)+"w" for i in range(1,sled_to_j+1)])) # Warm
...
def define_plotting(n_dims,n_sec,sled_to_j,lw,compare=False):
    ...
    if n_comp==1:  # 1 component
        ...
        plotinds.append(list(range(n_dims+np.sum(n_sec),n_dims+np.sum(n_sec)+sled_to_j,1)))
        ...
        plotinds.append(list(range(n_dims+np.sum(n_sec),n_dims+np.sum(n_sec)+2*sled_to_j,1)))
...
def post_sep(post_file):
    ...
    ind=list(range(2,len(blank)-1,2))
    breakind=[]
...
def get_dists(distfile,s,datsep,maxcind,grid_points=40):
    ...
    if redodists:
       ...
       # The secondary paramters (luminosity, pressure, bacd) are included here.
        ...
        print(list(datsep.keys()))
        for key in list(datsep.keys()):
...
def maketables(s,n_params,parameters,cube,add,mult,n_comp,title='results',addmass=0,n_dims=8):
    ...
    for j in s['modes']: 
        try:
            ...
            localev.append(j['local log-evidence'])
...
def plotmarginal(s,dists,add,mult,parameters,cube,plotinds,n_sec,n_dims,nicenames,n_mol,
    ...
    for g,j in enumerate(plotinds[0]):   
        gridinds=int(np.floor((np.mod(g,4))/nx)),np.mod(g,nx)
        if not dists2 and not simplify: # Don't do these if we are overplotting a 2nd dist, too confusing, or Simplify is set!
            for m,mode in enumerate(s['modes']):
                ...
                xx= np.ravel(list(zip(dists[m][j][0,:]*mult[j]+add[j], dists[m][j][0,:]*mult[j]+add[j] + dx)))
                yy =np.ravel(list(zip(yplot, yplot)))
...
def plotsled(meas,cube,n_params,n_dims,n_comp,modemed,modemax,modesig,plotinds,title='',lum=False,meas2=0,cube2=[],setyr=[0,0],
    ...
    if 'mol' not in list(meas.keys()):
        meas['mol']=np.array([meas['head']['mol'] for x in meas['flux']])
    ok=(meas['flux']!=0) & (meas['mol']==mol) #[meas['flux']!=0]
    ul=(meas['flux']==0) & (meas['mol']==mol) #[meas['flux']==0]
    ...
    if sled_to_j:
        ...
        if lum:     
            ...
            (yfill1,trash)=spire_conversions(yfill1,'kkms','lsol',list(range(1,14,1))*115.3,sr=meas['head']['omega_s'],
                              z=meas['head']['z'],dist=meas['head']['dl'])
            (yfill2,trash)=spire_conversions(yfill2,'kkms','lsol',list(range(1,14,1))*115.3,sr=meas['head']['omega_s'],
                              z=meas['head']['z'],dist=meas['head']['dl'])       
            yfill1*=meas['head']['lw']
            yfill2*=meas['head']['lw']
        
        if n_comp==1:    
            plt.fill_between(list(range(1,sled_to_j+1,1)), yfill1, yfill2, where=yfill2>=yfill1, 
                 facecolor='gray', interpolate=True,alpha=alpha)
        else:
            plt.fill_between(list(range(1,sled_to_j+1,1)), yfill1[0:sled_to_j], yfill2[0:sled_to_j], 
                where=yfill2[0:sled_to_j]>=yfill1[0:sled_to_j], facecolor='b', interpolate=True,alpha=alpha)
            plt.fill_between(list(range(1,sled_to_j+1,1)), yfill1[sled_to_j:], yfill2[sled_to_j:], 
    ...
    if meas2: 
        ok2=(meas2['flux']!=0) & (meas2['mol']==mol) #[meas2['flux']!=0]
        ul2=(meas2['flux']==0) & (meas2['mol']==mol) #[meas2['flux']==0]
    ...
    for w in w_in:
        ...
        dat['FLUX_Kkms']=np.array(list(map(float,dat['FLUX_Kkms'])))*np.power(10,thiscube[3])
        ...
        if t_n_comp==2:
            ...
            dat2=pyradex.pyradex(minfreq=1, maxfreq=1600,
                           ...
                           tbg=meas['tbg'], species=mol, velocity_gradient=1.0, 
                           debug=False,return_dict=True)
            dat2['FLUX_Kkms']=np.array(list(map(float,dat2['FLUX_Kkms'])))*np.power(10,thiscube[7])
            ...
            newdat=np.array(list(map(float,dat['FLUX_Kkms'])))+np.array(list(map(float,dat2['FLUX_Kkms'])))
...
def plotmarginalsled(s,dists,add,mult,parameters,cube,plotinds,n_sec,n_dims,n_comp,nicenames,n_mol,title='',norm1=True,
    ...
    for g,j in enumerate(plotinds[useind]):      
        gridinds=int(np.floor((np.mod(g,sled_to_j))/nx)),np.mod(np.mod(g,sled_to_j),nx)
    ...
    if dists2:
        ...
        try: 
            for g,j in enumerate(plotinds2[3]):
                gridinds=int(np.floor((np.mod(g,sled_to_j2))/nx)),np.mod(g,nx)
                yplot2=dists2['all'][j][1,:]
                axarr[gridinds].plot(dists2['all'][j][0,:]*mult2[j]+add2[j], yplot2, '-', color=colors2[j], drawstyle='steps')
                axarr[gridinds].axvline(x=cube2[j]*mult2[j]+add2[j],color=colors2[j],linestyle='-',label='4D Max, Comparison')          
        except:
...
def get_covariance(datsep):
    ...
    print('Finding covariance for modes: ',list(datsep.keys()))
    cov={}
...
def nbyn(nvars):
    ...
    return nrow,ncol,list(range(nvars,ncol*nrow))
